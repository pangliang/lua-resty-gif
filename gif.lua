...
while p(0) < body:len() do
    ...
    if block_flag == 0x21 then
        ...
        if ext_block_flag == 0xff then
            ...
            ngx.log(ngx.STDERR, "bflag:", string.format("%x,%x,%x,%x", bflag[1],bflag[2],bflag[3],bflag[4]))
            ngx.log(ngx.STDERR, "bflag:", string.sub(body, p(0), p(0)+20))
            if bflag[1] == 0x0b and string.sub(body, p(0)+1, p(0)+11) == "NETSCAPE2.0" then
                ...
                ngx.log(ngx.STDERR, string.format("sub_block_data_size:%x, sub_block_id:%x", sub_block_data_size, sub_block_id))
